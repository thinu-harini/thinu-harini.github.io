...
//           </head>
//           <body>
//             <div id="minimap-content">${contentRef.current.innerHTML}</div>
//           </body>
//         </html>
//       `);
//       iframeDoc.close();

//       // Adjust scaling after content load
//       iframeDoc.addEventListener('load', () => {
//         const contentDiv = iframeDoc.getElementById('minimap-content');
//         contentDiv.style.transform = `scale(${scale})`;
//         contentDiv.style.transformOrigin = '0 0';
//         contentDiv.style.width = `${100 / scale}%`;
//         contentDiv.style.height = `${100 / scale}%`;
//       });
//     };
...
//     //     minimapSizeRef.current.style.paddingTop = `${(contentRef.current.clientHeight / bodyWidth) * 100}%`;
//     //     viewerRef.current.style.paddingTop = `${(window.innerHeight / window.innerWidth) * 115}%`;
//     //   };
...
const Minimap = ({ contentRef, isVisible }) => {
  ...
  useEffect(() => {
    const getDimensions = () => {
      ...
      const bodyWidth = contentRef.current.clientWidth;
      const bodyHeight = contentRef.current.clientHeight;

      const minimapWidth = minimapRef.current.clientWidth;
      const minimapHeight = minimapRef.current.clientHeight;

      // Calculate scale to fit both dimensions
      const scaleWidth = minimapWidth / bodyWidth;
      const scaleHeight = minimapHeight / bodyHeight;
      const realScale = Math.min(scaleWidth, scaleHeight);

      setScale(realScale);

      minimapSizeRef.current.style.paddingTop = `${(bodyHeight / bodyWidth) * 100}%`;
      viewerRef.current.style.paddingTop = `${(window.innerHeight / window.innerWidth) * 115}%`;

      minimapContentRef.current.style.transform = `scale(${realScale})`;
      minimapContentRef.current.style.transformOrigin = '0 0';
      minimapContentRef.current.style.width = `${100 / realScale}%`;
      minimapContentRef.current.style.height = `${100 / realScale}%`;
    };
